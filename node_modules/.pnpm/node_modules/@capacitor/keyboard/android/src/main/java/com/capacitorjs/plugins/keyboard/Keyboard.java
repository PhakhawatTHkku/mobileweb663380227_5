...
public class Keyboard {
    ...
    private boolean shouldApplyEdgeToEdgeAdjustments() {
        // แก้ไขบรรทัดนี้ให้เป็นการดึงค่า String จาก Config
        String adjustMarginsForEdgeToEdge = (this.bridge == null)
                ? "auto"
                : this.bridge.getConfig().getString("adjustMarginsForEdgeToEdge", "auto");

        if ("force".equals(adjustMarginsForEdgeToEdge)) {
            return true;
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM && "auto".equals(adjustMarginsForEdgeToEdge)) {
            TypedValue value = new TypedValue();
            boolean optOutAttributeExists = activity
                    .getTheme()
                    .resolveAttribute(android.R.attr.windowOptOutEdgeToEdgeEnforcement, value, true);

            if (!optOutAttributeExists) {
                return true;
            } else {
                return value.data == 0;
            }
